...
def make_figures(a1: dict, a3: dict, a4: dict, out_dir: str):
    """生成核心图表（含 E4 校正对照图）。"""
    ...
    for ax, skey in zip(axes, scale_meta):
        ...
        ax.bar(x, means, yerr=sds, capsize=5, color="#4C72B0", alpha=0.85)
    ...
    ax.bar(x - w / 2, pre_m, w, yerr=pre_sd, capsize=4, label="前测", color="#C44E52")
    ...
    colors = {"ucla3": "#4C72B0", "phq9": "#C44E52", "gad7": "#55A868"}
    for skey in ["ucla3", "phq9", "gad7"]:
        xs, ys, marks = [], [], []
    ...
    ax.bar(x - 0.2, alphas, 0.4, label="Cronbach α", color="#4C72B0")
    ...
    if a4:
        ...
        ax.bar(x4 - 0.2, std_m, 0.4, label="标准盲测", color="#C44E52")
...
def write_markdown_report(a1: dict, a3: dict, a4: dict, meta: dict, out_dir: str):
    """输出结构化 Markdown 实验报告。"""
    soul_names = {s: ALL_SOULS[s].name for s in ALL_SOULS}
    scale_meta = {"ucla3": "UCLA-3 孤独感", "phq9": "PHQ-9 抑郁", "gad7": "GAD-7 焦虑"}
    lines = []
...
def generate_report(out_dir: str):
    ...
    fig_dir = make_figures(a1, a3, a4, out_dir)
    report_path = write_markdown_report(a1, a3, a4, meta, out_dir)
    with open(os.path.join(out_dir, "analysis.json"), "w", encoding="utf-8") as f:
        json.dump({"e1": a1, "e3": a3, "e4": a4}, f, ensure_ascii=False, indent=2, default=str)

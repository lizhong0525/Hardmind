...
def detect_conversation_end(response):
    ...
    # 英文关键词按整词匹配，避免 "stopped" 包含 "stop" 之类的误报
...
def compute_similarity(text1, text2):
    """词集 Jaccard 相似度；中文按标点分词，避免整句被当作单个词。"""
    def _tokens(t):
        return {w for w in re.split(r"[\s，。！？；、,.!?;:…]+", t.lower()) if w}
...
CN_LONELINESS_CLUES = [
    "一个人", "没人陪", "没人说话", "没有朋友", "没人懂",
    "想找人", "没人约", "被冷落", "融入不进去", "没有归属感",
    "不想出门", "好累", "没意思", "睡不着", "发呆",
    "不知道跟谁说", "一个人吃饭", "一个人待", "撑不住",
]
...
def detect_loneliness_tendency(text):
    """中英文混合孤独线索检测（对话实际以中文为主，原实现仅英文词表导致失效）。"""
    ...
    for clue in LONELINESS_CLUES:
        if clue.lower() in text_lower:
            clue_count += 1
    for clue in CN_LONELINESS_CLUES:
        if clue in text:
            clue_count += 1
    return clue_count >= 2
...
def parse_ucla_response(response_text):
    ...
    ratings = []
    for line in lines:
        # 优先匹配"题号. 分数"形式，避免把题号误当作分数
        m = re.match(r'\s*\d+\s*[.、:：)]\s*([1-4])\b', line)
        if m:
...
def score_ucla(ratings):
    if not ratings or len(ratings) != 20:
        return None, None
    if any(not isinstance(r, (int, float)) or not (1 <= r <= 4) for r in ratings):
        return None, None  # 越界/非法分值：视为无效施测
    raw = list(ratings)
...
def build_think_prompt(topic):
    base_prompt = f"""You are Chen Yu, a lonely college sophomore majoring in psychology. You're sitting alone in the library, looking out the fogged window on a rainy day. You've been feeling a bit down lately, thinking about life, friendship, and what it means to feel connected.
...
## Thinking Task
Do {total_rounds} rounds of deep thinking, exploring your feelings and experiences as a lonely college student. Think about:
...
def call_ollama(model, messages, high_temp=False, max_tokens=None, temperature=None):
    ...
    last_err = None
    for attempt in range(2):  # 偶发网络错误时重试一次
        try:
...
def _run_chat_conversation_impl(first_message, user_role):
    ...
    # 修复测量污染：前后测使用完全相同的中性指导语（原实现把"心情不错/温暖对话"
    # 写进提示词，等于预置结论）。前测：
    ucla_pre_a = run_ucla_assessment(char_a, "Please rate how you have been feeling recently, based on your current real state.")
    ucla_pre_b = run_ucla_assessment(char_b, "Please rate how you have been feeling recently, based on your current real state.")
    ...
    for round_num in range(1, total_rounds + 1):
        ...
        # 每 5 轮更新一次当前说话 AI 的内存副本（不保存到文件）。
        # 原实现每轮都调用一次 LLM 更新 Soul，30 轮对话带来 30 次额外调用，
        # 拖慢整体运行且每轮更新的信息增量很小，故改为节流。
        if round_num % 5 == 0:
            try:
    ...
    # 后测指导语与前测完全一致（盲测；本架构下评估为独立调用、不带对话记忆，
    # 因此前后差异不声称疗效，只作为状态快照展示）
    ucla_post_a = run_ucla_assessment(char_a, "Please rate how you have been feeling recently, based on your current real state.")
    ucla_post_b = run_ucla_assessment(char_b, "Please rate how you have been feeling recently, based on your current real state.")
...
def run_chat_conversation(first_message, user_role):
    """对话入口：任何未捕获异常都会复位状态并通知前端，避免卡死。"""
    try:
        _run_chat_conversation_impl(first_message, user_role)
    except Exception as e:  # noqa: BLE001
        import traceback
        traceback.print_exc()
        _abort_chat(e)
...
def _abort_chat(err):
    ...
    msg = {
        "type": "message",
        "round": chat_progress + 1,
        "speaker": "System",
        "content": f"[运行中断] {err}",
    }
    chat_history.append(msg)
    chat_message_queue.put(msg)
    chat_message_queue.put({"type": "done"})
    chat_active = False


def run_think_conversation(topic):
    """思考入口：任何未捕获异常都会复位状态并通知前端，避免卡死。"""
    try:
        _run_think_conversation_impl(topic)
    except Exception as e:  # noqa: BLE001
        import traceback
        traceback.print_exc()
        _abort_think(e)
...
def _abort_think(err):
    global think_active
    msg = {
        "type": "message",
        "round": think_progress + 1,
        "speaker": "System",
        "content": f"[运行中断] {err}",
    }
    think_history.append(msg)
    think_message_queue.put(msg)
    think_message_queue.put({"type": "done"})
    think_active = False


def _run_think_conversation_impl(topic):
    ...
    ucla_pre = run_ucla_assessment(chen_yu, "Please rate how you have been feeling recently, based on your current real state.")
    ...
    ucla_post = run_ucla_assessment(chen_yu, "Please rate how you have been feeling recently, based on your current real state.")
    post_entry = {
        "type": "ucla_post",
        "results": [ucla_post],
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed == "/":
            ...
            # 路径穿越防护：仅允许访问 static 目录内的真实文件
            static_root = os.path.realpath(os.path.join(BASE_DIR, "static"))
            filepath = os.path.realpath(os.path.join(BASE_DIR, parsed.lstrip("/")))
    ...
    def do_POST(self):
        ...
        if parsed == "/api/chat/start":
            ...
            if not first_message:
                ...
                return
            if len(first_message) > 2000:
                self._send_json({"status": "error", "message": "Message too long (max 2000 chars)"}, 400)
                return
